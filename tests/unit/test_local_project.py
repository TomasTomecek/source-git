...
def test_local_project_working_dir_project():
    ...
    project_mock = flexmock(
        ...
        namespace="namespace",
        service=flexmock(),
        get_git_urls=lambda: {"git": "git ssh"},
    )
...
def test_local_project_path_or_url_git_service():
    """No get_project on offline"""
    flexmock(path).should_receive("isdir").and_return(True).once()

    project = LocalProject(
        path_or_url="http://some/example/path",
        repo_name="repo_name",
        git_service=flexmock().should_receive("get_project").once().mock(),
    )
    assert project.repo_name == "repo_name"
    assert project.namespace == "example"
    assert project.git_service
    assert not project.git_project
